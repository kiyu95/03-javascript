...
// 하지만 자바스크립의 경우 하나의 숫자 타입만 존재하고 모든 수를 실수로 처리한다.
...
/*
    템플릿 리터럴
    Esc부터 도입 된 멀티라인 문자열, 표현식 삽입 등의 편리한 문자열 처리 기능을 제공하는 문자열 표기법
...
console.log(`안녕하세요 ${lastName} ${firstName} 입니다.`); // 위 표현식을 이렇게 작성 가능.

/*
    03. 불리언 타입
    불리언 타입의 값은 논리적 참, 거짓을 타나내는 true와 false 뿐이다.
*/
...
let test; // 변수를 선언한 이후 값을 할당하지 않은 변수를 참조하면 undefined가 반환
console.log(test);

// const test2; const는 상수 값이기 때문에 undefined으로 초기화 할 수 없음
...
/*
    ...
    자바스크립트 엔진은 누구도 참조하지 않는 메모리 공간에 대해 가바지 콜렉션을 수행하게 된다.
*/
...
/*
    ...
    다른 값과 중복 되지 않은 유일무이한 값으로 이름 충돌함 위험이 없는 객체에 유일한 프로퍼티 키를 만들기 위해서 사용한다.
